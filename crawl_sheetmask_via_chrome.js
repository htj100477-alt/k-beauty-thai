...
function fetchPageDom(page) {
  const url = `https://www.oliveyoung.co.kr/store/display/getMCategoryList.do?dispCatNo=${categoryCode}&fltDispCatNo=&prdSort=SW&pageIdx=${page}&rowsPerPage=48`;
  const cmd = `"${chromePath}" --headless --dump-dom "${url}"`;
  try {
    const stdout = execSync(cmd, { maxBuffer: 30 * 1024 * 1024, encoding: 'utf8' });
    return stdout;
  } catch (e) {
    return '';
  }
...
async function run() {
  console.log(`====================================================`);
  console.log(` Crawling Olive Young Category: Sheet Masks (${categoryCode}) via Chrome`);
  console.log(`====================================================\n`);
